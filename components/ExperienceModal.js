      ...
      <div className="flex flex-col">
        ...
        <div className="flex flex-wrap mt-3">
          {technologies.map((value, index) => {
            return (
              <div key={index} className="flex items-center bg-teal-400/10 hover:bg-teal-400/20 transition-all text-teal-300 text-xs w-min px-3 py-2 leading-5 rounded-full mr-1.5 mt-2">{value}</div>
            )
          })}
        </div>
      </div>
